...
public final class Unit4MyDatabase implements Unit4Database {
    ...
    public List<String> fiveLongFilms(FilmManager films) {
        return films.stream().filter(Film.LENGTH.greaterThan(120)).map(film -> film.getTitle()).limit(5).collect(Collectors.toList());
    }

    @Override
    public List<Film> filmsSortedByLengthThirdPage(FilmManager films) {
        return films.stream().sorted(Film.LENGTH).skip(20).limit(10).collect(Collectors.toList());
    }
